...
int main(int argc,char *argv[]) {
  ...
  if ( argc != 4 ) {
    ...
    printf("        vizuSolution.msh file: the nodal values of your function at"
 ...
 /** 2) Build mesh in MMG5 format */
  ...
  if ( MMG2D_saveMshMesh(mmgMesh,mmgSol,"vizuSolution.msh") != 1 ) {
    printf("Unable to save the solution for vizualization.\n");
    exit(EXIT_FAILURE);
  }
  ...
  /* Save msh file that contains the metric and the mesh for vizualisation */
  if ( MMG2D_saveMshMesh(mmgMesh,mmgSol,"vizuMet.msh") != 1 ) {
    printf("Unable to save the metric for vizualization.\n");
    exit(EXIT_FAILURE);
  }
